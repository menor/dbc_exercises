...
for (officer in voteCount) {
  
  // This block counts the votes
  for (elector in votes) {
    if (voteCount[officer][votes[elector][officer]])
      voteCount[officer][votes[elector][officer]]  += 1;
    else
      voteCount[officer][votes[elector][officer]] = 1;
  };

  // This block assigns the winner to officers
  var max = 0;
  for (electable in voteCount[officer]) {
    if (voteCount[officer][electable] > max) {
      officers[officer] = electable;
      max = voteCount[officer][electable];
    };
  };
